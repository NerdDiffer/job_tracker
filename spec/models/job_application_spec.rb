...
describe JobApplication, type: :model do
  ...
  describe '.get_record_val_by' do
    ...
    subject { build(:job_application, company: company, posting: posting, id: 2) }

    context "the subject" do
      it "has these attributes & values" do
        expect(subject).to have_attributes(
          active: true, title: 'Example Company - Chief Hot Pocket', id: 2
        )
      end
    ...
    context "retrieving real attributes from associated models" do
      before(:each) do
        allow(Company).to receive(:find_by_name).with(subject.company.name).and_return company
      end
      ...
      it "returns company name" do
        expect(Company.get_record_val_by(:name, subject.company.name, :return_attr => 'name')).to eq "Example Company"
      end
    end
    context "retrieving real attributes while searching by virtual attribute" do
      before(:each) do
        allow(JobApplication).to receive(:find_by_title).with(subject.title).and_return subject
      end
  ...
  describe '#title' do
    ...
    context "the most common case" do
      ...
      it "combines company.name with posting.job_title" do
        expect(subject.title).to eq "#{subject.company.name} - #{subject.posting.job_title}"
      end
    end
    context "job application not associated with any particular company" do
      ...
      it "combines a timestamp with the posting's job_title" do
        expect(subject.title).to eq "#{Time.now.strftime("%Y%m%d%H%M%S")} - #{subject.posting.job_title}"
      end
    end
    ...
    context "job application not associated with both a company and posting" do
      ...
      it "just returns a timestamp" do
        expect(subject.title).to eq "#{Time.now.strftime("%Y%m%d%H%M%S")}"
      end
    end
  end
